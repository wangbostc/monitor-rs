...
impl NetSampler {
    ...
    pub fn tick(&mut self) -> NetIo {
        // refresh(false): update the counter values on existing interfaces
        // without re-running the address/operational-state enumeration. The
        // re-enumeration variant (refresh(true)) was observed empirically to
        // yield near-zero deltas across consecutive ticks even under a saturated
        // download — keep this on `false`.
